...
def RQ2(dataset, datatype, modeltype, Iteration_Batchsize, remove_ratio, Mutation_Operation, Mutation_Epoch):
    ...
    if Mutation_Operation == 'direct':
        if remove_ratio == '005':
            ...
            for i in range(feaVec.shape[0]):
                ...
                new_feaVec.append(np.hstack((feaVec[i, 0:57], feaVec[i, 68], feaVec[i, 68])))
            ...
            feaVec = np.load(
                '../MTFL/features/randomweight_' + datatype + '_feature' + remove_ratio + '.npy',
                allow_pickle=True)
            indic_fea = -2
            indic_gt = -1
    ...
    if Mutation_Operation == 'randomweight':
        feaVec = np.load(
            '../MTFL/features/randomweight_' + datatype + '_feature.npy',
            allow_pickle=True)
        indic_fea = -2
        indic_gt = -1
    ...
    if datatype == 'D1':
        workbook_save_path = 'G:/dfauloV2/RQ2/' + dataset + '/' + Mutation_Epoch + '_' + Mutation_Operation + '_' + remove_ratio + '_' + str(
            Iteration_Batchsize) + '_' + 'LabelNoise' + modeltype + '.xls'
    elif datatype == 'D2':
        workbook_save_path = 'G:/dfauloV2/RQ2/' + dataset + '/' + Mutation_Epoch + '_' + Mutation_Operation + '_' + remove_ratio + '_' + str(
            Iteration_Batchsize) + '_' + 'DataNoise' + modeltype + '.xls'
...
if __name__ == "__main__":
    ...
    for conf in config:
        RQ2(dataset='MTFL', datatype='D1', modeltype='', Iteration_Batchsize=conf[3],
                     remove_ratio=conf[2],
