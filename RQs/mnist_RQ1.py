...
def RQ1(modeltype, datatype, excelx, sheet):
    ...
    np.save('F:/ICSEdata/RQ1data/MNIST/' + datatype + modeltype + '_offline.npy', savedata)
    X = [x for x in range(101)]
    ...
    plt.plot(X, y, color='red', label='Online')
    np.save('F:/ICSEdata/RQ1data/MNIST/' + datatype + modeltype + '_online.npy', onlinedata)
    ...
    np.save('F:/ICSEdata/RQ1data/MNIST/' + datatype + modeltype + '_gini.npy', ginidata)
    ...
    # np.save('F:/ICSEdata/RQ1data/MNIST/' + datatype + modeltype + '_cleanlab.npy', ginidata)#useless
    ...
    np.save('F:/ICSEdata/RQ1data/MNIST/' + datatype + modeltype + '_Coordinatedata.npy', Coordinatedata)

    plt.xlabel(r'$Percentage\ of\ test\ case\ executed$')
    plt.ylabel(r'$Percentage\ of\ fault\ detected$')
    plt.legend()
    if datatype == 'alllabel':
        plt.savefig('F:/ICSEdata/RQ1picture/' + 'RandomLabelNoise_' + modeltype + '.jpg', dpi=1200)  # 指定分辨率保存
    elif datatype == 'ranlabel':
        plt.savefig('F:/ICSEdata/RQ1picture/' + 'SepcificLabelNoise_' + modeltype + '.jpg', dpi=1200)  # 指定分辨率保存
    elif datatype == 'alldirty':
        plt.savefig('F:/ICSEdata/RQ1picture/' + 'RandomDataNoise_' + modeltype + '.jpg', dpi=1200)  # 指定分辨率保存
    elif datatype == 'randirty':
        plt.savefig('F:/ICSEdata/RQ1picture/' + 'SpecificDataNoise_' + modeltype + '.jpg', dpi=1200)  # 指定分辨率保存
    return sheet
...
if __name__ == "__main__":
    feaVec = np.load('F:/ICSEdata/RQ1usedata/MNISTfeature/alllabelLeNet1_feature.npy',
                     allow_pickle=True)  # LeNet5
    print(feaVec.shape)
    AT(feaVec, 'LeNet1', 'alllabel')
