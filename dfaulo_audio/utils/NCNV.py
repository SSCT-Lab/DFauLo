...
def ncnv(net, eval_loader, num_class, batch_size, feat_dim=512, num_neighbor=20):
    ...
    for batch_idx, (inputs, labels, _) in enumerate(eval_loader):
...
def nclc(net, net2, labeled_trainloader, unlabeled_trainloader, testloader, threshold_scor, batch_size, num_class,
         feat_dim=512, num_neighbor=20, high_scor=1.0):
    net.eval()
    net2.eval()

    # loading labeled samples
    labeledFeatures, labeledLogits, labeledNoisyLabels, labeledW = getFeature(net, net2, labeled_trainloader,
                                                                              testloader, feat_dim, num_class)
    ...
    unlabeledFeatures, unlabeledLogits, _, _ = getFeature(net, net2, unlabeled_trainloader, testloader, feat_dim,
                                                          num_class)
...
def train(net, net2, optimizer, labeled_trainloader, unlabeled_trainloader, pseudo_labels, batch_size, num_class):
    net.train()
    ...
    num_iter_labeled = (len(labeled_trainloader.dataset) // batch_size) + 1
    num_iter_unlabeled = (len(unlabeled_trainloader.dataset) // batch_size) + 1
    ...
    for batch_idx in range(num_iter):
        ...
        try:
            inputs_uw, inputs_uw2, inputs_us, labels_u, _, _, index_u = next(unlabeled_train_iter)
        except:
            unlabeled_train_iter = iter(unlabeled_trainloader)
            inputs_uw, inputs_uw2, inputs_us, labels_u, _, _, index_u = next(unlabeled_train_iter)

        # transforming given label to one-hot vector for labeled samples
        targets_x = torch.zeros(inputs_xw.size(0), num_class).scatter_(1, labels_x.view(-1, 1), 1)
        ...
        mixed_input, mixed_target = mixup(mixed_inputs, mixed_targets, alpha=4)
        if num_class==4:
            # transform to LongTensor
            mixed_input = mixed_input.type(torch.cuda.LongTensor)
        ...
        #     all_inputs, all_labels, all_masks = torch.cat([inputs_us, inputs_xs], dim=0), torch.cat(
        #         [labels_u, labels_x], dim=0), torch.cat([mask_u, mask_x], dim=0)
        ...
        print(
            "\rIter[%3d/%3d]\t Lx: %.4f, Lu: %.4f, Lpen: %.4f"
            %(batch_idx + 1, num_iter, Lx.item(),
               Lu.item(), penalty.item()),end=""
        )
