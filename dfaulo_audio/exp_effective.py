...
# RandomLabelNoise
# dataset_path = "./AudioClassification-Pytorch/dataset/train_list_rln_features.txt"
class_path = "./AudioClassification-Pytorch/dataset/classes.json"
model_path = "./AudioClassification-Pytorch/models/rln/TDNN_Fbank/last_model/model.pth"


# 处理数据格式为dfaulo的输入格式
# save_path = "./AudioClassification-Pytorch/dataset/train_rln/train"
# if not os.path.exists(save_path):
#     os.makedirs(save_path)
#     with open(class_path, 'r') as f:
#         classes = json.load(f)
#     for key in classes.keys():
#         os.makedirs(os.path.join(save_path, str(key)))
#     classes_dict = {int(v): k for k, v in classes.items()}
#     with open(dataset_path, 'r') as f:
#         lines = f.readlines()

#     for line in lines:
#         source_path = "./AudioClassification-Pytorch/" + line.split("\t")[0]
#         category_id = eval(line.split("\t")[1])
#         # copy source file to save_path
#         save_dir = classes_dict[category_id]
#         save_dir = os.path.join(save_path, save_dir)
#         os.system(f"cp {source_path} {save_dir}")
...
parser.add_argument('--dataset', default="./AudioClassification-Pytorch/dataset/train_rln", help='input dataset')
parser.add_argument('--model', default=model_path,
                    help='input model path')
parser.add_argument('--model_name', default=Model, help='input model path')
parser.add_argument('--class_path', default=class_path,
                    help='input model path')
...
data_s = data_slice(args, args.dataset+"/train", args.slice_num)
