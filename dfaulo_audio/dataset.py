...
class MAClsDataset(Dataset):
    def __init__(self,
        ...
        self.data_list_path = data_list_path+"/train"
        data_list_path = data_list_path+"/train"
        self.max_duration = 3
        ...
        # 获取数据列表
        if label_spc is None:
            self.lines = []
            self.labels = []
            for class_name in classes.keys():
                path = os.path.join(data_list_path, class_name)
                files = os.listdir(path)
                for file in files:
                    line = os.path.join(self.data_list_path, class_name, file)
                    if line not in ignore_list:
                        self.lines.append(line)
                        self.labels.append(classes[class_name])
            print(f"数据集中的数据有{len(self.lines)}条")
            
        if label_spc is not None:
            self.lines = []
            self.labels = []
            path = os.path.join(data_list_path, label_spc)
            files = os.listdir(path)
            for file in files:
                line = os.path.join(self.data_list_path, label_spc, file)
                self.lines.append(line)
                self.labels.append(classes[label_spc])
            print(f"数据集中的数据有{len(self.lines)}条")
    ...
    def __getitem__(self, idx):
        # 分割数据文件路径和标签
        data_path = self.lines[idx]
        label = self.labels[idx]
        # 如果后缀名为.npy的文件，那么直接读取
        if data_path.endswith('.npy'):
            feature = np.load(data_path)
            if feature.shape[0] > self.max_feature_len:
                crop_start = random.randint(
                    0, feature.shape[0] - self.max_feature_len) if self.mode == 'train' else 0
                feature = feature[crop_start:crop_start +
                                  self.max_feature_len, :]
            feature = torch.tensor(feature, dtype=torch.float32)

        label = torch.tensor(int(label), dtype=torch.int64)
        return feature, label, data_path
