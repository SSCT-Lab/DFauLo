...
def FeatureExtraction(datatype):
    ...
    def get_featureV(data, orgsfm, GTsfm, S1sfm, S2sfm, S3sfm, orgloss, S1loss, S2loss, S3loss, suslist, datatype):
        ...
        feaV = np.array(feaV)
        # sum1 = 0
        # for i in range(feaV.shape[0]):
        #     if int(feaV[i, -11]) == 1:
        #         sum1 += 1
        # print('dirty num:', sum1)
...
def AT(feaVec, datatype):
    ...
    #     rank.append([int(isdt[i]), float(LRres[i])])
    newdata = []
...
def UPDATE(feaVec, datatype):
    ...
    la = -1
    _, _, _, _, _, _, _, _, _, bb_left, fea_left = AT(feaVec, datatype)
...
def Uncertainty(feaVec, datatype):
    model = TCDCNN()
    orgstate_dict = torch.load('../models/' + datatype + '_tcdcn.pth')
    ...
    for i in range(feaVec.shape[0]):  # 57 66
        orgdata.append([feaVec[i][67], feaVec[i][57], feaVec[i][58], feaVec[i][59],
                        feaVec[i][60], feaVec[i][61], feaVec[i][62], feaVec[i][63],
                        feaVec[i][64], feaVec[i][65], feaVec[i][66]])
    orgdata = np.array(orgdata)
    bb = np.loadtxt('../MTFL/TCDCN_NEWDATA_OUT/annotation_' + datatype + '_VAE_MID_OUT_new.txt',
                    dtype=str)
    lm = []
    for i in range(orgdata.shape[0]):
    ...
    with torch.no_grad():
        for i, img in tqdm(enumerate(imageset)):
            ...
            output_list = []
            for j in range(T):
                output_list.append(model(img).numpy()[0])
            output_list = np.array(output_list)
            var = output_list.var(0)  # 方差
            unct_list.append(var.mean())
    print(len(unct_list))
    rank = []
    for i in range(feaVec.shape[0]):
        rank.append(int(feaVec[i][68]))
    ...
    newbb = []
    for i in range(feaVec.shape[0]):
        newbb.append([bb[i][0], bb[i][1], bb[i][2], bb[i][3], float(unct_list[i])])
...
if __name__ == "__main__":
    ...
    # ORGLABEL('D2')
    # datatype='org'
    # feaVec = np.load('../MTFL/features/' + datatype + '_feature_new.npy',
    #                  allow_pickle=True)  # LeNet5
    # random.seed(6657)
    # AT(feaVec,datatype)
    # FeatureExtraction('org')
    # dtlist = ['D1', 'D2']
    # datasetname = 'MTFL'
    ...
    #     row += 1
    #     sheet = RQ1(DATATYPE, row, sheet)
    # workbook.save('F:/ICSEdata/excel/' + datasetname + 'newclus_RQ1.xls')
